...
def run_once(worker_id: str = 'analyze-1', limit: int = 5) -> int:
    ...
    for kit in kits:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                with connection() as conn:
                    with conn.transaction():
                        # Update kit
                        execute(conn, "UPDATE kits SET analysis_state = 'analyzed', analyzed_at = now() WHERE id = %s", (kit_id,))
                        ...
                        for inv_file in inventory:
                            execute(
                                ...
                                (kit_id, inv_file.path, inv_file.sha256, inv_file.tlsh, inv_file.normalized_token_hash, inv_file.size, inv_file.mime, inv_file.is_obfuscated)
                            )
                        ...
                        for ind in all_indicators:
                            execute(
                                ...
                                (kit_id, ind.type.value, ind.value_hash, ind.redacted_display, ind.full_value_encrypted, ind.confidence, ind.found_in_path, "{}" ) # Serialize meta properly in real app
                            )
                        ...
                        execute(
                            ...
                            (kit_id, fingerprint.fileset_hash, fingerprint.antibot_hash, fingerprint.token_hash, fingerprint.author_strings, fingerprint.file_sha_set)
                        )
            ...
            with connection() as conn:
                execute(conn, "UPDATE kits SET analysis_state = 'error', analysis_error = %s WHERE id = %s", (error_msg, kit_id))
